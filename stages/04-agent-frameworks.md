...
| | [vercel/eve](https://github.com/vercel/eve) | ⭐⭐⭐ | 想在 TS/JS 生態建長駐、可維運的 agent；喜歡「檔案即介面」的專案結構 | Vercel 出品的 filesystem-first framework：agent 的組成都是慣例位置的檔案——`instructions.md`（系統提示）、`tools/`、`skills/`、`channels/`（HTTP / Slack / Discord）、`schedules/`（排程），好檢視、好擴充。`npx eve@latest init` 起手、Apache-2.0、TypeScript，★ 4.3k+。⚠️ 2026-06 才發布、很新 |
